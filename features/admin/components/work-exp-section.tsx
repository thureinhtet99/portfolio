...
export default function WorkExperienceSection() {
  ...
  const [workExpToDelete, setWorkExpToDelete] = useState<string | null>(null);
  ...
  const handleEdit = (timeline: WorkExperienceType) => {
    setEditingId(timeline.id);

    setWorkForm({
      companyName: timeline.companyName,
      companyLogo: timeline.companyLogo || "",
      companyWebsite: timeline.companyWebsite || "",
      positions: timeline.positions?.map((p) => ({
        id: p.id,
        title: p.title,
  ...
  return (
    <Card>
      <AdminSectionHeader
        ...
        count={workTimelines.length}
        onAdd={() => {
          setIsAdding(!isAdding);
          setEditingId(null);
          resetForm();
        }}
        addLabel="Add Work exp"
      />
...
function WorkCard({
  ...
  timeline: WorkExperienceType;
  onEdit: (timeline: WorkExperienceType) => void;
